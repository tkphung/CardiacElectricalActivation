function [H,varargout] = FE_RenderPatchMesh(NODES,ECON,varargin)
...
if data_flag % WITH DATA
    ...
    p = patch('Vertices',NODES(:,1:3),'Faces',HEXes,...
          'FaceVertexCData',repmat(dataCOL(:),6,1),'FaceColor','flat',...
          'EdgeColor',edgecolor); hold on 
    else
    p = patch('Vertices',NODES(:,1:3),'Faces',HEXes,...
          'FaceVertexCData',repmat(dataCOL(:),6,1),'FaceColor','flat'); hold on 
    end
else % WITHOUT DATA
    if edgecolor_flag
    p = patch('Vertices',NODES(:,1:3),'Faces',HEXes,...
