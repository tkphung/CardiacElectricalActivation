...
% 
% MODIFIED by TNP (11-22-2017) to export polygonal data; cell data; and a
% lookup table colormap
...
%                 fwrite(fid, reshape(varargin{sidx(ii)+2},1,n_elements),'float','b');
    ...
    case 'POLYDATA'
        ...
        fprintf(fid, ['\nCELL_DATA ' num2str(num)]);
        % Parse remaining argument.
        vidx = find(strcmpi(varargin,'VECTORS'));
        sidx = find(strcmpi(varargin,'SCALARS'));
        lidx = find(strcmpi(varargin,'COLORMAP'));
        zidx = find(strcmpi(varargin,'COLORSCALARS'));
        if vidx~=0
            for ii = 1:length(vidx)
                title = varargin{vidx(ii)+1};
                % Data enteries begin with a keyword specifying data type
        ...
        if sidx~=0
            for ii = 1:length(sidx)
                ...
                if lidx == 0
                    ...
                    fprintf(fid,['LOOKUP_TABLE ',varargin{lidx+1},'\n']);
                end
                if ~binaryflag
                    spec = ['%0.', precision, 'f ','\n'];
                    scalardata = varargin{ sidx(ii) + 2}(:);
                    fprintf(fid, spec, scalardata);
                else
                    fwrite(fid, varargin{ sidx(ii) + 2}, 'float', 'b');
                end
%                 fwrite(fid, reshape(varargin{sidx(ii)+2},1,n_elements),'float','b');
        ...
        if lidx~=0
            for ii = 1:length(lidx)
                lut_name = varargin{lidx(ii)+1};
                % Data enteries begin with a keyword specifying data type
                % and numeric format.
                coloring = (varargin{lidx(ii)+ 2});
                fprintf(fid, ['\nLOOKUP_TABLE ', lut_name,' ',num2str(size(coloring,1)),'\n']);
                spec = ['%0.', precision, 'f ','%0.', precision, 'f ','%0.', precision, 'f ','%0.', precision, 'f ','\n'];
                fprintf(fid,spec,coloring');
            end
        end
        if zidx~=0
            for ii = 1:length(zidx)
                lut_name = varargin{zidx(ii)+1};
                % Data enteries begin with a keyword specifying data type
                % and numeric format.
                coloring = (varargin{zidx(ii)+ 2});
                fprintf(fid, ['\nCOLOR_SCALARS ', lut_name,' ',num2str(size(coloring,1)*4),'\n']);
                spec = ['%0.', precision, 'f ','%0.', precision, 'f ','%0.', precision, 'f ','%0.', precision, 'f ','\n'];
                fprintf(fid,spec,coloring');
            end
        end
