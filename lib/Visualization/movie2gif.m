...
% mov=importdata(mov); % added by TK 2-11-16
...
if (nargin > 2)
    h = waitbar(0, 'Generate GIF file...');
    frameNb = size(mov, 2);
%     frameNb = 60; %Hardcoded by TK (20 FPS for 3 second simulation (PostView fixed time step of 0.05s))
    isFirst = true;
    for i = 1:frameNb
%     for i = 1:60 %Hardcoded by TK for 1 cardiac cycle
        waitbar((i-1)/frameNb, h);
        [RGB, ~] = frame2im(mov(i));
        if (exist('rgb2ind', 'file'))
            [IND, map] = rgb2ind(RGB,256);
