function [H,varargout] = FE_RenderPentMesh(NODES,ECON,varargin)
...
% edgealpha flag
% Updated by Thien-Khoi N. Phung (September 19, 2019)- removed for loop
% around patch for faster function!
% From FE_RenderPatchMesh- To plot the PENT meshes
...
if data_flag
    dataCOL = DATA;% DO NOT NORMALIZING DATA ./max(DATA);
else
    if ~facecolor_flag
...
HEX = ECON(plotelements,:);
face = [2 3 6 5; 1 2 5 4; 3 1 4 6];
HEXes = [];
for jz = 1:size(face,1)
    HEXes = [HEXes; HEX(:,face(jz,:))];
end

face = [1 2 3; 4 5 6];
for jz = 1:size(face,1)
    HEXes = [HEXes; HEX(:,face(jz,:)) nan(size(HEX(:,face(jz,:)),1),1)];
end

if data_flag % WITH DATA
    if edgecolor_flag
    p = patch('Vertices',NODES(:,1:3),'Faces',HEXes,...
          'FaceVertexCData',repmat(dataCOL(:),5,1),'FaceColor','flat',...
          'EdgeColor',edgecolor); hold on 
    else
    p = patch('Vertices',NODES(:,1:3),'Faces',HEXes,...
          'FaceVertexCData',repmat(dataCOL(:),5,1),'FaceColor','flat'); hold on 
    end
else % WITHOUT DATA
    if edgecolor_flag
    p = patch('Vertices',NODES(:,1:3),'Faces',HEXes,...
