function H = MC_HammerMap_Scar(STACK,varargin)
...
% Cartesian XYZ 
XYZ = data(:,1:3);
SCAR = data(:,4:5); % transmurality (%) & wall thickness (mm)
...
Hscar = griddata(the,mu,SCAR(:,1),Hthe,Hmu);
Hscar(isnan(Hscar)) = 0;
...
contlines=[-0.3 0 0.1 0.3 0.5 0.7 0.9];

% Plot SCAR
H = figure('Name',plottitle,'NumberTitle','off','WindowStyle',windowstyle); hold on
[C, h] = contourf(HamX, HamY, Hscar, contlines);
    set(h,'LineWidth',2)
...
if segment_flag
    plot(HamsegX,HamsegY,'b.','MarkerSize',8)
end

if savefig_flag
    saveas(gcf,[savepath 'scar hammer map ' date '.fig']);
end
...
BullseyeMu=[0:1:120]';
Hscar(:,361)=Hscar(:,1);
Z_subtract_bullseye=[Hscar(:,181:361) Hscar(:,1:180)];
...
figure('Name','Bullseye Scar','NumberTitle','off','WindowStyle',windowstyle); hold on
polar([0 2*pi],[0 100]);
...
if savefig_flag
    saveas(gcf,[savepath 'scar bullseye map ' date '.fig']);
end
