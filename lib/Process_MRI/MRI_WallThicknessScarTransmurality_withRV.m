function ProcessedStack = MRI_WallThicknessScarTransmurality_withRV(ProcessedStack,scar_flag)
...
% CHANGED TNP March 17, 2017- altered function for LV+RV meshing
...
for i = ProcessedStack.Slices;
    ...
    % CHANGED THIS SECTION TO USE EPI CENTROID (TNP 03-17-2017)- mean(endo.
    endo_shift = [endo(:,1)-mean(endo(:,1)),endo(:,2)-mean(endo(:,2)),endo(:,3)];
    epi_shift = [epi(:,1)-mean(endo(:,1)),epi(:,2)-mean(endo(:,2)),epi(:,3)];
    if scar_flag
