function [Cxyz,M,varargout] = MRI_RotateScarStack(setstruct,varargin)
%MRI_RotateScarStack: Rotates scar contours
% [Cxyz,M,varargout] = MRI_RotateScarStack(setstruct,varargin)
%   INPUT VARIABLE:
...
% 
% Thien-Khoi N. Phung (Based on PlotSAScarStack.m- January 18, 2017)
...
for idx=1:(imaging_planes)
    ...
    if length(slice_labels) > 1
        for jdx=slice_labels
            [Xs,M] = MRI_TransformScarStack(imgstruct,jdx);
            Xd = cell2mat(Xs);
        ...
        [Xs,M] = MRI_TransformScarStack(imgstruct);
        Xd = cell2mat(Xs);
