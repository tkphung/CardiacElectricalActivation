function [xyz_pts,M] = MRI_TransformEpiStack(imgstruct,varargin)
...
% all)
% Previous function name: TransformSAEpiStack.m

if ~isempty(varargin)
    slicenumber = varargin{1};
    x_pix = imgstruct.EpiX(:,:,slicenumber);
    y_pix = imgstruct.EpiY(:,:,slicenumber);
else
    slicenumber = 1;
    x_pix = imgstruct.EpiX(:,:,slicenumber);
    y_pix = imgstruct.EpiY(:,:,slicenumber);
