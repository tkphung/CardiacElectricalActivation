function ProcessedStack = MRI_ProcessStack(SA,LA,varargin)
%MRI_ProcessStack: Takes in segmented MRI (using Segment) in a short axis
...
%      *STEP CORRECTION pre-5: Elimate the tracings in other time frame
...
% REMOVE SHIFT IN CENTERS OF SLICES (TNP May 2, 2018)
...
% *Edited by TNP (July 18, 2017)
% *Edited Step 8f & 11 by TNP (May 2, 2018)
...
if scar_flag
    % Restructure Scar Variable to be similar to Endo + Epi
    % ALTERED BY TNP 04-13-2017: Segment saves only the *.Scar.Result
    % instead of *.Scar.Manual when you use the autosegmentation. Therefore
    % we will check if the *.Scar.Manual has any scar segmentation and IF
    % NOT then we will copy over the *.Scar.Result to  *.Scar.Manual
    
    % changed to less than or equal (from equal) because manual exclusion
    % of .Result scar shows up as -1 regions in the .Manual
    if sum(sum(sum(SAstack.Scar.Manual))) <= 0 
        SAstack.Scar.Manual = SAstack.Scar.Result;
    end   
    ...
    for wh = 1:SAstack.ZSize
        [x,y] = find(SAstack.Scar.Manual(:,:,wh) == 1);
        SAstack.Scar.ScarX(1:length(x),1,wh) = x;
        SAstack.Scar.ScarY(1:length(y),1,wh) = y;
    end

    %Rotate Scar Contours
    [Cxyz_SAScar,~] = MRI_RotateScarStack(SAstack);
    for jz = 1:SAstack.ZSize
        slice_ind = Cxyz_SAScar(:,5)==jz;
...
% REMOVE SHIFT IN SLICES (TNP May 2, 2018)
% % 8f: Shift center of slice to correspond to base-apex axis
count=0;
for i = SAstack.KeptSlices
    count = count+1;
    ...
    slice_center(i,:) = mean(transform_epi(slice_idx_epi,:));
    axis_center(i,:) = BAaxis_intersect_pts(count,:);
%     center_axis_diff(i,:) = slice_center(i,:)-axis_center(i,:);
    center_axis_diff(i,:) = [0 0 0]; % Changed (TNP May 2, 2018)
...
% xlabel('x'); ylabel('y'); zlabel('z');
% title('Slices Shifted so Slice X-Y Center (Calculated from EPI Contours) Lies on Apex-Base Axis');
...
for i = slices_all
    ...
    polar_center = [mean(xepicart),mean(yepicart)]; % This should match slice_center
    % center_diff = [polar_center(1,1)-ProcessedStack.SliceCenters(i,1),polar_center(1,2)-ProcessedStack.SliceCenters(i,2)];
    center_diff = polar_center - slice_center(i,1:2); % Change by TNP (May 2, 2018);
